...
export async function handleOffline(request:Request,env:Env):Promise<Response|null>{
 ...
 if((path==='/api/v1/offline/challenge'||path==='/api/v1/offline/refresh')&&request.method==='POST'){
  try{
   const text=await request.text();if(text.length>12000)return reply({success:false,message:'请求过大'},413);
   const body=JSON.parse(text);if(typeof body.deviceId!=='string'||body.deviceId.length>200)return reply({success:false,code:'DEVICE_NOT_BOUND'},400);
  ...
  }catch{return reply({success:false,message:'请求无效'},400);}
 }
 if(path!=='/api/v1/offline/issue'||request.method!=='POST')return null;
 const secret=(env as Env & {OFFLINE_RSA_PRIVATE_KEY?:string}).OFFLINE_RSA_PRIVATE_KEY;
 if(!secret)return reply({success:false,message:'离线激活尚未配置，请联系管理员。'},503);
 ...
 try{
  const raw=await request.text();if(raw.length>12000)return reply({success:false,message:'请求过大'},413);
  const body=JSON.parse(raw),r=String(body.request||''),licenseKey=String(body.licenseKey||'').trim().toUpperCase();
  ...
  if(q.v!==2||q.app!=='CleanC'||!/^\w{32}$/.test(q.sessionId)||typeof q.deviceId!=='string'||q.deviceId.length>200||!q.deviceId||typeof q.devicePublicKey!=='string'||q.devicePublicKey.length>2000||!Number.isSafeInteger(q.createdAt)||q.createdAt>now||now-q.createdAt>600000||typeof q.box!=='string'||q.box.length>600)throw Error('format');
  const key=await crypto.subtle.importKey('pkcs8',decode(secret.replace(/-----BEGIN PRIVATE KEY-----|-----END PRIVATE KEY-----|\s/g,'')),{name:'RSA-OAEP',hash:'SHA-256'},false,['decrypt']);
  const clear=new Uint8Array(await crypto.subtle.decrypt({name:'RSA-OAEP'},key,decode(q.box)));
  ...
  if(!reserved)return reply({success:false,message:'此二维码已使用或正在处理，请在电脑重新生成二维码。'},409);
  ...
  if(!result.ok){await env.DB.prepare('DELETE FROM offline_activation_sessions WHERE session_id=? AND status=?').bind(q.sessionId,'pending').run();clear.fill(0);return result;}
  ...
  const code=shortCode(packed);clear.fill(0);
  await env.DB.prepare("UPDATE offline_activation_sessions SET status='issued' WHERE session_id=?").bind(q.sessionId).run();
  await env.DB.prepare('DELETE FROM offline_activation_sessions WHERE expires_at<?').bind(now-86400000).run();
  return reply({success:true,code:code.match(/.{4}/g)!.join('-'),expiresAt:type==='permanent'?null:new Date(Date.UTC(2020,0,1)+minutes*60000).toISOString(),licenseType:type});
 }catch{return reply({success:false,message:'二维码无效、已超时或电脑时间不准确，请校准时间并重新生成。'},400);}
}
export const offlinePage=`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CleanC 离线激活</title><style>*{box-sizing:border-box}body{margin:0;min-height:100svh;display:grid;place-items:center;padding:24px;font:16px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#173052;background:radial-gradient(at 0 0,#d2ecff,transparent 60%),radial-gradient(at 100% 80%,#e8dfff,transparent 60%),#f4f8ff}main{width:100%;max-width:460px;padding:32px;background:#ffffffbe;backdrop-filter:blur(30px);border:1px solid white;border-radius:30px;box-shadow:0 24px 80px #294b7520}input,button{width:100%;font:inherit;padding:16px;border-radius:15px;margin:8px 0;border:1px solid #d4deed}button{background:#2673e6;color:white;cursor:pointer;min-height:48px}button:disabled{opacity:.5}p{line-height:1.7;color:#62728a}output{display:block;font-size:24px;font-weight:700;overflow-wrap:anywhere;margin:20px 0}input:focus-visible,button:focus-visible{outline:3px solid #438cff;outline-offset:3px}</style><main><small>CLEANC · LICENSE</small><h1>离线激活</h1><p>输入授权码，获取当前电脑专用的 16 位激活码。与在线激活采用相同授权类型和到期时间。</p><form id="form"><label for="key">授权码</label><input id="key" autocomplete="off" maxlength="200" placeholder="CLC-XXXX-XXXX-XXXX-XXXX" required><button id="submit">确认授权</button></form><output id="result" aria-live="polite"></output><p id="message">无需登录管理后台。二维码十分钟内有效。</p></main><script>const r=location.hash.slice(1);history.replaceState(null,'',location.pathname);document.getElementById('form').addEventListener('submit',async e=>{e.preventDefault();const b=document.getElementById('submit'),m=document.getElementById('message');b.disabled=true;try{const response=await fetch('/api/v1/offline/issue',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({request:r,licenseKey:document.getElementById('key').value}),cache:'no-store'});const data=await response.json();if(!response.ok||!data.success)throw Error(data.message||'激活失败');document.getElementById('result').textContent=data.code;document.getElementById('key').value='';m.textContent='请在电脑点击“我已扫码”，输入以上激活码。'+(data.expiresAt?'有效期至 '+new Date(data.expiresAt).toLocaleString():'永久授权');document.getElementById('form').hidden=true}catch(err){m.textContent=err.message}finally{b.disabled=false}});</script></html>`;
