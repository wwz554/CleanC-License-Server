...
<style>
*{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#17233b;background:radial-gradient(circle at 5% 8%,#d9efff,transparent 34%),radial-gradient(circle at 92% 4%,#eadfff,transparent 38%),linear-gradient(135deg,#f8fbff,#eef4ff);min-height:100vh}.glass{background:rgba(255,255,255,.68);backdrop-filter:blur(28px) saturate(150%);border:1px solid rgba(255,255,255,.88);box-shadow:0 20px 60px rgba(43,72,140,.12),inset 0 1px 0 #fff;border-radius:24px}.app{min-height:100vh;padding:20px}.side{position:fixed;top:20px;bottom:20px;width:230px;padding:22px;display:flex;flex-direction:column}.brand{display:flex;align-items:center;gap:10px}.logo{width:44px;height:44px;border-radius:15px;display:grid;place-items:center;font-size:20px;font-weight:800;color:#fff;background:linear-gradient(145deg,#4a8cff,#765cff)}.nav{margin-top:22px}.nav-btn{display:block;width:100%;text-align:left;margin:7px 0;background:transparent;color:#58667e}.nav-btn.active{background:#ffffffd9;color:#315fc5}.main{margin-left:250px}.muted{color:#72809a}.btn{border:0;border-radius:13px;padding:10px 14px;cursor:pointer;font-weight:650;font:inherit}.btn:disabled{opacity:.58;cursor:not-allowed}.primary{background:linear-gradient(135deg,#4d88ff,#735cff);color:#fff}.secondary{background:#eef3ff;color:#375baf}.danger{background:#fff0f2;color:#c73555}.success{background:#ecf9f1;color:#24794a}.logout{margin-top:auto;width:100%}input,select{font:inherit;width:100%;padding:12px 14px;border:1px solid #dce5f4;border-radius:13px;background:#ffffffdf}.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.row>*{flex:1}.row .btn{flex:0 0 auto}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}.card,.panel{padding:20px}.panel{margin-top:16px}.stat b{font-size:30px}.section{display:none}.section.active{display:block}.hidden{display:none!important}.notice{padding:12px 14px;border-radius:13px;background:#eef4ff;color:#4c628f;margin:12px 0}.error{padding:12px 14px;border-radius:13px;background:#fff0f2;color:#b83250;margin:12px 0}.ok{padding:12px 14px;border-radius:13px;background:#ecf9f1;color:#24794a;margin:12px 0}table{width:100%;border-collapse:collapse;font-size:14px}th,td{text-align:left;padding:10px 8px;border-bottom:1px solid #e4eaf3;vertical-align:top}code{background:#eef3ff;padding:3px 6px;border-radius:7px;word-break:break-all}.actions{display:flex;gap:6px;flex-wrap:wrap}.pill{display:inline-block;padding:3px 8px;border-radius:999px;background:#eef3ff;color:#375baf;font-size:12px}.toolbar{display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap}.status{min-height:20px;margin-top:10px}@media(max-width:900px){.side{position:static;width:auto;margin-bottom:14px}.main{margin-left:0}.grid{grid-template-columns:repeat(2,1fr)}.app{padding:12px}.logout{margin-top:16px}}@media(max-width:560px){.grid{grid-template-columns:1fr}.row>*{flex-basis:100%}}
</style>
</head>
<body>
<div class="app">
  ...
  <main class="main">
    ...
    <section id="dashboard" class="section active">
      <div class="grid">
        <div class="card glass stat">总授权<br><b id="sTotal">-</b></div>
        <div class="card glass stat">有效授权<br><b id="sActive">-</b></div>
        <div class="card glass stat">已禁用<br><b id="sDisabled">-</b></div>
        <div class="card glass stat">绑定设备<br><b id="sDevices">-</b></div>
      </div>
    </section>

    <section id="licenses" class="section">
      <div class="panel glass">
        <div class="notice"><b>生产规则：</b>每个授权码同一时间只能绑定 1 台设备；每台设备同一时间只能占用 1 个授权。管理员解绑后可重新绑定。</div>
        <div class="row">
          <input id="customKey" placeholder="自定义授权码（留空随机）">
          <select id="licenseType"><option value="permanent">永久</option><option value="duration">激活后 N 天</option><option value="fixed">固定到期</option></select>
          <input id="durationDays" type="number" value="7" min="1" max="36500" placeholder="有效天数">
          <input id="expiresAt" class="hidden" type="datetime-local">
          <input id="batchCount" type="number" value="1" min="1" max="15" placeholder="生成数量">
          <button id="createLicenseBtn" class="btn primary">生成授权</button>
        </div>
        <div style="height:10px"></div>
        <input id="licenseNote" placeholder="备注（可选）">
        <div id="licenseStatus" class="status muted"></div>
      </div>
      <div class="panel glass" style="overflow:auto"><table><thead><tr><th>授权码</th><th>状态</th><th>类型</th><th>首次激活</th><th>到期</th><th>设备</th><th>创建</th><th>操作</th></tr></thead><tbody id="licenseRows"></tbody></table></div>
    </section>

    <section id="devices" class="section">
      <div class="panel glass"><div class="notice">授权总期限到期后，客户端通过 challenge + refresh 验证设备身份并安全释放旧绑定；管理员也可在这里手动解绑。</div></div>
      <div class="panel glass" style="overflow:auto"><table><thead><tr><th>设备</th><th>授权码</th><th>Windows</th><th>App</th><th>最近在线</th><th>状态</th><th>操作</th></tr></thead><tbody id="deviceRows"></tbody></table></div>
    </section>
    ...
    <section id="settings" class="section">
      <div class="panel glass">
        <h3>域名与 API</h3>
        <p>当前访问：<code id="currentOrigin">-</code></p>
        <p>Bootstrap：<code id="bootstrapUrl">-</code></p>
        <p>主授权地址：<code id="canonicalUrl">-</code></p>
        <div class="notice">先在 Cloudflare Pages 绑定 Custom Domain，然后在这里保存。保存时只需要再次输入管理员密码。</div>
        <div class="row"><input id="newDomain" placeholder="license.example.com"><input id="confirmPassword" type="password" placeholder="管理员密码（二次确认）"></div>
        <div style="height:12px"></div>
        <button id="saveDomainBtn" class="btn primary">检测并保存</button>
        <button id="rollbackDomainBtn" class="btn danger">恢复 Bootstrap 地址</button>
        <div id="settingsStatus" class="status muted"></div>
      </div>
      <div class="panel glass"><h3>生产客户端认证流程</h3><div class="notice">首次激活 1 次；之后本地验证服务器签名 Lease。正常情况下仅在 Lease 到期前每约 72 小时执行 <b>challenge + refresh</b> 两次请求。</div><div id="apiList"></div></div>
    </section>
  </main>
</div>
<script>
